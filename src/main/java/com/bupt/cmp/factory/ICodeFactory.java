...
import com.bupt.cmp.ast.java.TokenMgrError;
import com.bupt.cmp.lang.Language;
import com.bupt.cmp.lang.file.DefaultCodeFile;
import com.bupt.cmp.pretreat.PretreatDirector;
...
//            System.out.println(defaultCodeFile.getContent());
            ...
            if(error instanceof TokenMgrError){
                log.error("初始化defaulCodeFile出错！: "+file.getAbsolutePath()+">"+error);
            }else if(error instanceof com.bupt.cmp.ast.cpp.TokenMgrError){
                log.error("初始化defaulCodeFile出错！: "+file.getAbsolutePath()+">"+error);
            }else {
                log.error("初始化defaulCodeFile出错！ !"+file.getAbsolutePath()+">"+error);
            }
    ...
    public DefaultCodeFile generateDefectCodeFile(String code){
        ...
        try {
            ...
            if(error instanceof TokenMgrError){
                log.error("初始化defaulCodeFile出错！ "+code+">"+error);
            }else if(error instanceof com.bupt.cmp.ast.cpp.TokenMgrError){
                log.error("初始化defaulCodeFile出错！"+code+">"+error);
            }else {
                log.error("初始化defaulCodeFile出错！"+code+">"+error);
