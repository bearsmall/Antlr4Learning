...
public class SuffixArrayCore {
    ...
    public static int[] generateSA(List<SingleToken> tokens) {
        SIZE = calculateN(tokens.size());
        int[] wa = new int[SIZE];
        int[] wb = new int[SIZE];
        int[] wv = new int[SIZE];
        int[] ws = new int[SIZE];
        SingleToken[] tk = new SingleToken[tokens.size()];
        tokens.toArray(tk);
        int n = tk.length;
        int m = SIZE;
        int[] x = wa;
    ...
    public static int[] generateH(List<SingleToken> tokens, int[] sa) {
        SingleToken[] ch = new SingleToken[tokens.size()];
        tokens.toArray(ch);
        int n = sa.length;
        int[] h = new int[n+1];
